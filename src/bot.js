...
function getStorageClients() {
  ...
  return {
    tableService: TableServiceClient.fromConnectionString(connStr),
    stateTable: TableClient.fromConnectionString(connStr, 'state'),
  };
}

async function ensureTable(tableService, tableClient) {
  await tableService.createTable('state').catch((err) => {
    if (err.statusCode !== 409) throw err;
  });

  try {
    await tableClient.getEntity('state', 'novel');
  } catch {
...
async function sendTweet(twitter, text, context) {
  context.log(text);
  try {
    await twitter.v2.tweet(text);
  } catch (error) {
    context.error('Failed to send tweet:', error);
  }
}

async function doWork(context) {
  const twitter = getTwitterClient();
  const { tableService, stateTable } = getStorageClients();

  await ensureTable(tableService, stateTable);

  const die = Math.floor(Math.random() * 2);

  switch (die) {
    case 0: {
      let index = 0;
      try {
        const state = await stateTable.getEntity('state', 'novel');
        index = state.index ?? 0;
      } catch {
        // Default to 0
      }

      index = (index + 1) % novel.length;

      await stateTable.updateEntity(
        { partitionKey: 'state', rowKey: 'novel', index },
        'Merge'
      );

      context.log(`Tweeting novel index: ${index}`);
      await sendTweet(twitter, novel[index], context);
      break;
    }

    case 1: {
      const phrase = Math.floor(Math.random() * misc.length);
      context.log(`Tweeting miscellaneous quote index: ${phrase}`);
      await sendTweet(twitter, misc[phrase], context);
      break;
    }
  }
}
